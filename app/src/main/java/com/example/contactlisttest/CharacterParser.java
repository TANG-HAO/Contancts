...
public  class CharacterParser {
     ...
     */
    public String getSelling(String datum) {
        String key, value;
        buffer = new StringBuilder();
        for (int i = 0; i < datum.length(); i++) {
            key = datum.substring(i, i + 1);
            System.out.println("key========>"+key);
            if (key.getBytes().length >= 2) {  //判断是中文字符还是英文
        ...
        System.out.println("buffer.toString()=====>"+buffer.toString());//todo 中文都变成zuozuo  已解决
            return buffer.toString();
        }
    /** * 单字解析 * * @param str * @return */
    private Object convert(String key) {
        String result=null;
        int ascii=getChsAscii(key);
    ...
    private int getChsAscii(String key) {
        int asc = 0;
        try {
